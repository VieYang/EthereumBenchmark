...
// newTestAccount generates a new key and stores it into walletPath,
// encrypting it with the walletPassword
...
func main() {
	...
	// creates a keystore for walletPath
	wallet := keystore.NewKeyStore(*walletPath,
		keystore.LightScryptN, keystore.LightScryptP)
	...
	if *getBalance == true {
		for _, account := range wallet.Accounts()[1:] {
			balance, err := client.BalanceAt(ctx, account.Address, nil)
			if err != nil {
				fmt.Println("Balance error:", err)
				os.Exit(1)
			}
			fmt.Println(account.Address.Hex(), "Balance: ", balance.String())
		}
		os.Exit(0)
	}

	// get the account nonce of the given account.
	...
	for i := int64(0); i < *numOfTx; i++ {
		...
		fmt.Println("nonce: ", nonce, amount)
